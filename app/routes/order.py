...
def add_order_endpoint(order: OrderCreate, db: Session = Depends(get_db_session), current_user: dict = Depends(require_role("admin", "client"))):
    return create_order(order, db, current_user)

@router.put("/{id}", response_model=OrderRead)
def update_order_endpoint(id: int, order_update: OrderUpdate, db: Session = Depends(get_db_session), current_user: dict = Depends(require_role("admin", "client"))):
    return update_order(id, order_update, db, current_user)

@router.delete("/{id}")
def delete_order_endpoint(id: int, db: Session = Depends(get_db_session), current_user: dict = Depends(require_role("admin", "client"))):
